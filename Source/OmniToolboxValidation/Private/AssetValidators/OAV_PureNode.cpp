...
namespace UE::Internal::PureNodeValidatorHelpers
{
	bool IsHarmlessPureNode(UK2Node_CallFunction* CallNode)
	{
		...
		if (Func->HasMetaData(TEXT("NativeBreakFunc")) || Func->HasMetaData(TEXT("NativeMakeFunc")) || Func->HasMetaData(TEXT("IgnorePureNodeValidator")))
		{
			return true;
		}
		
		/**There are a ton of conversion nodes that are cheap enough and
		 * shouldn't be problematic enough to cause any issues.
		 * Let's ignore them. They all start with "Conv_"
		 */
		if(Func->GetAuthoredName().Contains("Conv_"))
		{
			return true;
		}
...
bool UOAV_PureNode::OmniValidatorCanValidateAsset(const FAssetData& InAssetData, UObject* InObject,
	FDataValidationContext& InContext) const
{
	return InObject != nullptr && InObject->IsA<UBlueprint>();
}

EDataValidationResult UOAV_PureNode::ValidateLoadedAsset_Implementation(const FAssetData& InAssetData, UObject* InAsset,
            ...
            if (UE::Internal::PureNodeValidatorHelpers::WillPureNodeFireMultipleTimes(CallNode, Graph))
            {
                ...
                const FText Message = FText::Format(
                    NSLOCTEXT("PureNodeValidator", "MultiCallWarning",
                              "{0} will execute more than once. Right click the node and `Convert to exec` or avoid connections to multiple inputs."),
                    Title
                );
                CallNode->ErrorMsg            = Message.ToString();
                CallNode->ErrorType           = EMessageSeverity::Error;
                ...
                TokenMessage->AddToken(
                    FActionToken::Create(
                        ...
                        {
                            UOmniValidationSettings::OpenBlueprintAndFocusNode(Blueprint, Graph, CallNode);
                        }),
                        /*bEnabled=*/false
                    )
                );
